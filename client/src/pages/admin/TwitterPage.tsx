...
import type { TwitterPost, TwitterPostStatus } from '@shared/types'
...
const STATUS_COLORS: Record<TwitterPostStatus, string> = {
  draft: 'bg-amber-100 text-amber-800',
  published: 'bg-green-100 text-green-800',
  failed: 'bg-red-100 text-red-800',
}

const STATUS_LABELS: Record<TwitterPostStatus, string> = {
  draft: 'Borrador',
  published: 'Publicado',
  failed: 'Fallido',
}

type Filter = 'all' | TwitterPostStatus
...
export default function TwitterPage() {
  ...
  const query = useQuery({
    queryKey: ['twitterPosts', filter, page],
    queryFn: () =>
      adminApi.twitter.listPosts({
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => adminApi.twitter.deletePost(id),
    onSuccess: () => {
      toast('success', 'Registro eliminado')
      queryClient.invalidateQueries({ queryKey: ['twitterPosts'] })
    },
    onError: (err) => toast('error', err instanceof Error ? err.message : 'Error al eliminar'),
  })

  const [panelOpen, setPanelOpen] = useState(false)
  const [selectedDraft, setSelectedDraft] = useState<TwitterPost | null>(null)
  const [publishing, setPublishing] = useState(false)

  const handleOpenDraft = useCallback((post: TwitterPost) => {
  ...
  const handleRefreshMetrics = useCallback(async () => {
    setRefreshing(true)
    try {
      await adminApi.twitter.refreshMetrics()
  ...
  function handleDelete(post: TwitterPost) {
    const message = post.status === 'published'
      ? `¿Eliminar este registro?\n\n"${post.postText.slice(0, 80)}..."\n\nEl post seguirá publicado en X: solo se borra el registro local.`
  ...
  return (
    <>
      ...
      <TwitterDraftPanel
        ...
        onPublish={async (postId) => {
          ...
          try {
            ...
            toast('error', err instanceof Error ? err.message : 'Error al publicar en X')
          } finally {
            setPublishing(false)
          }
        }}
      ...
      <PageHeader
        title="X/Twitter"
        description="Publicaciones generadas y publicadas en X"
        actions={
      ...
      {query.isLoading ? (
        ...
        <ErrorState message="Error al cargar las publicaciones de X" />
      ) : posts.length === 0 ? (
        <EmptyState
          title="Sin publicaciones"
          description="No hay publicaciones de X en esta categoría. Los borradores se generan desde la ficha de una noticia."
        />
      ) : (
        <>
...
function PostRow({
  ...
  post: TwitterPost
  onOpen: (post: TwitterPost) => void
  onDelete: (post: TwitterPost) => void
  isDeleting: boolean
}) {
    ...
    <tr className="hover:bg-neutral-50 transition-colors">
      ...
      <td className="px-4 py-3 text-right text-xs text-neutral-500 hidden lg:table-cell whitespace-nowrap tabular-nums">
        {post.status === 'published'
          ? `${post.likeCount}♥ ${post.retweetCount}RT ${post.replyCount}R`
          : '—'}
      </td>
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex items-center justify-end gap-1">
          {/* Only `draft` is publishable here: X publishes from draft alone, so a
              failed post is deleted and regenerated instead of retried. */}
          {post.status === 'draft' && (
            <ActionIconButton icon={PencilIcon} label="Editar/Publicar" onClick={() => onOpen(post)} />
          )}
          {post.status === 'failed' && (
            <ActionIconButton icon={PencilIcon} label="Ver el error" onClick={() => onOpen(post)} />
          )}
          {post.tweetUrl && (
            <a
              href={post.tweetUrl}
